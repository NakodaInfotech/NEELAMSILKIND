...
Public Class DNRegisterDaily
    ...
    Private Sub DNRegisterDaily_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub DNRegisterDaily_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
    ...
    Sub fillgrid()
        ...
        If chkdate.CheckState = CheckState.Unchecked Then
            dt = OBJCOMMON.search(" SUM(DR)-SUM(CR)", "", " Register_Grouped", " and name = '" & DNREGNAME & "' and acc_billdate <'" & Format(FromDate, "MM/dd/yyyy") & "' and acc_cmpid = " & CmpId & " and acc_LOCATIONid = " & Locationid & " and YEARID = " & YearId)
        Else
            dt = OBJCOMMON.search(" SUM(DR)-SUM(CR)", "", " Register_Grouped", " and name = '" & DNREGNAME & "' and acc_billdate <'" & Format(dtfrom.Value.Date, "MM/dd/yyyy") & "' and acc_cmpid = " & CmpId & " and acc_LOCATIONid = " & Locationid & " and YEARID = " & YearId)
        End If
        ...
        If Val(txtopening.Text.Trim) = 0 Then
            dt = OBJCOMMON.search("ACC_OPBAL, ACC_DRCR", "", "LEDGERS", " AND ACC_CMPNAME = '" & DNREGNAME & "' AND ACC_CMPID = " & CmpId & " AND ACC_LOCATIONID = " & Locationid & " AND ACC_YEARID = " & YearId)
            If dt.Rows.Count > 0 Then
                txtopening.Text = Val(dt.Rows(0).Item(0).ToString)
    ...
    Private Sub ExcelExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ExcelExport.Click
        Try
            ...
            PATH = Application.StartupPath & "\Debit Note Register (Daily).XLS"
            ...
            opti.SheetName = "Debit Note Register (Daily)"
            gridregister.ExportToXls(PATH, opti)
            EXCELCMPHEADER(PATH, "Debit Note Register (Daily)", gridregister.VisibleColumns.Count + gridregister.GroupCount, DNREGNAME, PERIOD, Val(txtopening.Text.Trim) & " " & lbldrcropening.Text.Trim, Val(txttotal.Text.Trim) & " " & lbldrcrclosing.Text)
    ...
    Private Sub TOOLDETAILS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TOOLDETAILS.Click
        Try
            Dim OBJCN As New DNRegisterDetails
    ...
    Private Sub DNRegisterDaily_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
