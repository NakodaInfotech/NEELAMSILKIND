...
Public Class SaleRegister
    ...
    Private Sub SaleRegister_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub SaleRegister_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            fillgrid()
        Catch ex As Exception
            Throw ex
    ...
    Sub fillgrid()
        Try
            ...
            Dim OBJSALEREG As New ClsSaleRegister
            ...
            OBJSALEREG.alParaval = ALPARAVAL
            DT = OBJSALEREG.getdata()

            If DT.Rows.Count > 0 Then
                For Each DTROW As DataRow In DT.Rows
                    gridregister.Rows.Add(MonthName(DTROW(0)), Format(Val(DTROW("DEBIT").ToString), "0.00"), Format(Val(DTROW("CREDIT").ToString), "0.00"), "0.00", Format(Val(DTROW("ST").ToString), "0.00"), "0.00")
                Next
            End If
    ...
    Sub total()
        Try
            ...
            For Each row As DataGridViewRow In gridregister.Rows
                ...
                row.Cells(GNETT.Index).Value = Val(row.Cells(cr.Index).Value) - Val(row.Cells(griddebit.Index).Value) - Val(row.Cells(GST.Index).Value)
                TXTST.Text = Format((Val(TXTST.Text.Trim) + Val(row.Cells(GST.Index).Value)), "0.00")
                TXTNETT.Text = Format((Val(TXTNETT.Text.Trim) + Val(row.Cells(GNETT.Index).Value)), "0.00")
    ...
    Sub SHOWFORM()
        Try
            Dim objSALEdetails As New SaleRegisterDetails
            objSALEdetails.SALEREGNAME = cmbregister.Text.Trim
            objSALEdetails.FromDate = getfirstdate(CmpId, gridregister.CurrentRow.Cells(0).Value.ToString)
            objSALEdetails.ToDate = getlastdate(CmpId, gridregister.CurrentRow.Cells(0).Value.ToString)
            objSALEdetails.MdiParent = MDIMain
            objSALEdetails.Show()
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Sub PrintToolStripButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles PrintToolStripButton.Click
        Try
            ...
            objreg.frmstring = "SaleRegisterMonthly"
            objreg.reg = cmbregister.Text.Trim
            If chkdate.Checked = True Then
                objreg.CHECK = 1
                objreg.FROMDATE = dtfrom.Value.Date
                objreg.TODATE = dtto.Value.Date
                objreg.PERIOD = "SALE REGISTER MONTHLY - " & Format(dtfrom.Value, "dd/MM/yyyy") & " - " & Format(dtto.Value, "dd/MM/yyyy")
            Else
                objreg.CHECK = 0
                objreg.FROMDATE = AccFrom
                objreg.TODATE = AccTo
                objreg.PERIOD = "SALE REGISTER MONTHLY - " & Format(AccFrom.Date, "dd/MM/yyyy") & " - " & Format(AccTo.Date, "dd/MM/yyyy")
            End If
    ...
    Private Sub SaleRegister_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
