...
Public Class ContraRegisterDaily
    ...
    Private Sub ContraRegisterDaily_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
        Try
    ...
    Private Sub ContraRegisterDaily_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
    ...
    Sub fillgrid()
        ...
        If chkdate.CheckState = CheckState.Unchecked Then
            dt = OBJCOMMON.search(" SUM(DR)-SUM(CR)", "", " Register_Grouped", " and name = '" & CONREGNAME & "' and acc_billdate <'" & Format(FromDate, "MM/dd/yyyy") & "' and acc_cmpid = " & CmpId & " and acc_LOCATIONid = " & Locationid & " and YEARID = " & YearId)
        Else
            dt = OBJCOMMON.search(" SUM(DR)-SUM(CR)", "", " Register_Grouped", " and name = '" & CONREGNAME & "' and acc_billdate <'" & Format(dtfrom.Value.Date, "MM/dd/yyyy") & "' and acc_cmpid = " & CmpId & " and acc_LOCATIONid = " & Locationid & " and YEARID = " & YearId)
        End If
        ...
        If Val(txtopening.Text.Trim) = 0 Then
            dt = OBJCOMMON.search("ACC_OPBAL, ACC_DRCR", "", "LEDGERS", " AND ACC_CMPNAME = '" & CONREGNAME & "' AND ACC_CMPID = " & CmpId & " AND ACC_LOCATIONID = " & Locationid & " AND ACC_YEARID = " & YearId)
            If dt.Rows.Count > 0 Then
                txtopening.Text = Val(dt.Rows(0).Item(0).ToString)
    ...
    Private Sub ExcelExport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles ExcelExport.Click
        Try
            ...
            PATH = Application.StartupPath & "\Contra Register (Daily).XLS"
            ...
            opti.SheetName = "Contra Register (Daily)"
            gridregister.ExportToXls(PATH, opti)
            EXCELCMPHEADER(PATH, "Contra Register (Daily)", gridregister.VisibleColumns.Count + gridregister.GroupCount, CONREGNAME, PERIOD, Val(txtopening.Text.Trim) & " " & lbldrcropening.Text.Trim, Val(txttotal.Text.Trim) & " " & lbldrcrclosing.Text)
    ...
    Private Sub TOOLDETAILS_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TOOLDETAILS.Click
        Try
            Dim OBJCON As New ContraRegisterDetails
    ...
    Private Sub ContraRegisterDaily_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
