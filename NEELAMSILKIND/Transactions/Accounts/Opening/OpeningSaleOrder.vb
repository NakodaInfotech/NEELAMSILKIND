...
Public Class OpeningSaleOrder
    ...
    Private Sub cmdOK_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdOK.Click
        Try
            ...
            Dim OBJOPSO As New ClsOpeningSaleOrder()
            OBJOPSO.alParaval = alParaval
            ...
            If EDIT = False Then
                If USERADD = False Then
                    ...
                    Exit Sub
                End If
                Dim DT As DataTable = OBJOPSO.SAVE()
    ...
    Sub getmax_OPSO_no()
        ...
        DTTABLE = getmax(" isnull(max(OPSO_no),0) + 1 ", "OPENINGSALEORDER", " AND OPSO_yearid=" & YearId)
    ...
    Private Function ERRORVALID() As Boolean
        ...
        If Val(TXTSONO.Text.Trim) > 0 And EDIT = False And TXTSONO.ReadOnly = False Then
            DTTABLE = OBJCMN.search("  ISNULL(OPSO_no, 0) AS SONO ", "", " OPENINGSALEORDER ", "  AND OPENINGSALEORDER.OPSO_no=" & TXTSONO.Text.Trim & " AND OPENINGSALEORDER.OPSO_CMPID = " & CmpId & " AND OPENINGSALEORDER.OPSO_LOCATIONID = " & Locationid & " AND OPENINGSALEORDER.OPSO_YEARID = " & YearId)
...
line1:
    ...
    Private Sub OPENINGSALEORDER_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If EDIT = True Then
                ...
                Dim OBJOPSO As New ClsOpeningSaleOrder()
                Dim DT As DataTable = OBJOPSO.SELECTSO(TEMPSONO, YearId)
                If DT.Rows.Count > 0 Then
    ...
    Private Sub OPENINGSALEORDER_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Try
...
LINE1:
    ...
    Sub PRINTREPORT()
        Try
            ...
            OBJsaleOrder.FORMULA = "{OPENINGSALEORDER.OPSO_no}=" & Val(TXTSONO.Text) & " and {OPENINGSALEORDER.OPSO_yearid}=" & YearId
            OBJsaleOrder.Show()
        Catch ex As Exception
            Throw ex
        End Try
    ...
    Private Sub TOOLCLOSE_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles TOOLCLOSE.Click
        Try
            Dim OBJOPSO As New SaleOrderClose
            OBJOPSO.MdiParent = MDIMain
            OBJOPSO.Show()
        Catch ex As Exception
            Throw ex
    ...
    Private Sub OPENINGSALEORDER_Shown(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Shown
        Try
            fillcmb()
        Catch ex As Exception
            Throw ex
    ...
    Private Sub TXTSONO_Validating(ByVal sender As System.Object, ByVal e As System.ComponentModel.CancelEventArgs) Handles TXTSONO.Validating
        Try
            If Val(TXTSONO.Text.Trim) <> 0 And EDIT = False Then
                Dim OBJCMN As New ClsCommon
                Dim dttable As DataTable = OBJCMN.search("  ISNULL(OPSO_no, 0) AS SONO ", "", " OPENINGSALEORDER ", "  AND OPENINGSALEORDER.OPSO_no=" & Val(TXTSONO.Text.Trim) & " AND OPENINGSALEORDER.OPSO_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
    ...
    Private Sub CMBDESIGN_Validated(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles CMBDESIGN.Validated
        Try
            If CMBDESIGN.Text.Trim <> "" Then
                Dim OBJCMN As New ClsCommon
                Dim DT As DataTable = OBJCMN.search("ISNULL(ITEM_NAME,'') AS ITEMNAME", "", " DESIGNMASTER INNER JOIN ITEMMASTER ON DESIGN_ITEMID = ITEM_ID", " AND DESIGN_NO = '" & CMBDESIGN.Text.Trim & "' AND DESIGN_YEARID = " & YearId)
    ...
    Private Sub TXTCOPYSONO_Validated(sender As Object, e As EventArgs) Handles TXTCOPYSONO.Validated
        Try
            If Val(TXTCOPYSONO.Text.Trim) > 0 And EDIT = False Then
                ...
                Dim OBJOPSO As New ClsOpeningSaleOrder()
                Dim DT As DataTable = OBJOPSO.selectSo(Val(TXTCOPYSONO.Text.Trim), YearId)
