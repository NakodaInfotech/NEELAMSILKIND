...
Public Class YarnRecdFromDyeing
    ...
    Dim GRIDDOUBLECLICK, GRIDUPLOADDOUBLECLICK, GRIDSOURCEDOUBLECLICK As Boolean
    Public EDIT As Boolean          'used for editing
    Public TEMPYARNNO As Integer     'used for poation no while editing
    Dim TEMPROW, TEMPUPLOADROW, TEMPSOURCEROW As Integer
    ...
    Sub CLEAR()
        ...
        CMBGODOWN.Text = "YARN DYEING GODOWN"
        ...
        TXTSOURCESRNO.Text = 1
        CMBSOURCEYARNQUALITY.Text = ""
        CMBSOURCECOLOR.Text = ""
        TXTSOURCEWT.Clear()

        CHKREDYEING.CheckState = CheckState.Unchecked

        GRIDSOURCE.Enabled = False
        TXTSOURCESRNO.Visible = False
        CMBSOURCEYARNQUALITY.Visible = False
        CMBSOURCECOLOR.Visible = False
        TXTSOURCEWT.Visible = False


        GRIDYARN.RowCount = 0
        GRIDPROG.RowCount = 0
        GRIDSOURCE.RowCount = 0

        CMDSELECTPROG.Enabled = True
        GRIDDOUBLECLICK = False
        GRIDUPLOADDOUBLECLICK = False
        GRIDSOURCEDOUBLECLICK = False
    ...
    Sub TOTAL()
        Try
            ...
            For Each ROW As DataGridViewRow In GRIDSOURCE.Rows
                If ROW.Cells(SSRNO.Index).Value <> Nothing Then
                    LBLSOURCETOTALWT.Text = Format(Val(LBLSOURCETOTALWT.Text) + Val(ROW.Cells(SWT.Index).EditedFormattedValue), "0.00")
                End If
            Next


            If CHKREDYEING.CheckState = CheckState.Unchecked And GRIDSOURCE.RowCount > 0 Then GRIDSOURCE.Rows(0).Cells(SWT.Index).Value = Format(Val(LBLTOTALWT.Text.Trim), "0.00")
        ...
        Try
            If YARNDATE.Text.Trim <> "__/__/____" Then
                ...
                If Not DateTime.TryParse(YARNDATE.Text, TEMP) Then
                    ...
                    Exit Sub
                Else
                    If EDIT = False Then dtpchallan.Value = YARNDATE.Text
                End If
            End If
        Catch ex As Exception
            Throw ex
    ...
    Sub GETMAXNO()
        ...
        DTTABLE = getmax(" isnull(max(YARN_no),0) + 1 ", "YARNRECDDYEING", " and YARN_yearid=" & YearId)
    ...
    Function ERRORVALID() As Boolean
        Try
            ...
            If GRIDSOURCE.RowCount = 0 Then
                EP.SetError(TabControl1, "Fill Source Details")
                bln = False
            End If
...
CHECKNEXTLINE:
    ...
    Private Sub cmdok_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdok.Click
        Try
            ...
            Dim SOURCEYARNQUALITY As String = ""
            Dim SOURCECOLOR As String = ""
            Dim SOURCEWT As String = ""

            For Each row As System.WINDOWS.FORMS.DataGridViewRow In GRIDSOURCE.Rows
                If row.Cells(0).Value <> Nothing Then
                    If SOURCESRNO = "" Then
                        SOURCESRNO = row.Cells(SSRNO.Index).Value.ToString
                        SOURCEYARNQUALITY = row.Cells(SYARNQUALITY.Index).Value.ToString
                        SOURCECOLOR = row.Cells(SCOLOR.Index).Value.ToString
                        SOURCEWT = Val(row.Cells(SWT.Index).Value)

                    Else
                        SOURCESRNO = SOURCESRNO & "|" & row.Cells(SSRNO.Index).Value
                        SOURCEYARNQUALITY = SOURCEYARNQUALITY & "|" & row.Cells(SYARNQUALITY.Index).Value.ToString
                        SOURCECOLOR = SOURCECOLOR & "|" & row.Cells(SCOLOR.Index).Value.ToString
                        SOURCEWT = SOURCEWT & "|" & Val(row.Cells(SWT.Index).Value)

                    End If
                End If
            Next

            alParaval.Add(SOURCESRNO)
            alParaval.Add(SOURCEYARNQUALITY)
            alParaval.Add(SOURCECOLOR)
            alParaval.Add(SOURCEWT)


            Dim objclsGRN As New ClsYarnRecdFromDyeing()
...
LINE1:
    ...
    Sub SAVEUPLOAD()
        ...
        Try
            Dim OBJBILL As New ClsYarnRecdFromDyeing
    ...
    Private Sub YarnRecdFromDyeing_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            If EDIT = True Then
                ...
                dttable = OBJYARN.SELECTYARN(TEMPYARNNO, CmpId, Locationid, YearId)
                ...
                Dim OBJCMN As New ClsCommon
                dttable = OBJCMN.search(" YARNRECDDYEING_UPLOAD.YARN_SRNO AS GRIDSRNO, YARNRECDDYEING_UPLOAD.YARN_REMARKS AS REMARKS, YARNRECDDYEING_UPLOAD.YARN_NAME AS NAME, YARNRECDDYEING_UPLOAD.YARN_PHOTO AS IMGPATH ", "", " YARNRECDDYEING_UPLOAD ", " AND YARNRECDDYEING_UPLOAD.YARN_NO = " & TEMPYARNNO & " AND YARN_YEARID = " & YearId & " ORDER BY YARNRECDDYEING_UPLOAD.YARN_SRNO")
                If dttable.Rows.Count > 0 Then
                ...
                'ORDER GRID
                'Dim OBJCMN As New ClsCommon
                dttable = OBJCMN.search(" YARNRECDDYEING_PROGDETAILS.YARN_GRIDSRNO AS GRIDSRNO, YARNQUALITYMASTER.YARN_name AS YARNQUALITY, ISNULL(COLORMASTER.COLOR_name, '') AS COLOR, ISNULL(YARNRECDDYEING_PROGDETAILS.YARN_PROGWT,0) AS PROGWT, YARNRECDDYEING_PROGDETAILS.YARN_FROMNO AS FROMNO, YARNRECDDYEING_PROGDETAILS.YARN_FROMSRNO AS FROMSRNO, YARNRECDDYEING_PROGDETAILS.YARN_FROMTYPE AS FROMTYPE, ISNULL(YARNRECDDYEING_PROGDETAILS.YARN_WT,0) AS RECDWT ", "", " YARNRECDDYEING_PROGDETAILS INNER JOIN YARNQUALITYMASTER ON YARNRECDDYEING_PROGDETAILS.YARN_YARNQUALITYID= YARNQUALITYMASTER.YARN_id LEFT OUTER JOIN COLORMASTER ON YARNRECDDYEING_PROGDETAILS.YARN_COLORID = COLORMASTER.COLOR_id ", " AND YARNRECDDYEING_PROGDETAILS.YARN_NO = " & TEMPYARNNO & " AND YARNRECDDYEING_PROGDETAILS.YARN_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
                    For Each DTR As DataRow In dttable.Rows
                ...
                'SOURCE GRID
                'Dim OBJCMN As New ClsCommon
                dttable = OBJCMN.search(" YARNRECDDYEING_SOURCEDETAILS.YARN_GRIDSRNO AS GRIDSRNO, YARNQUALITYMASTER.YARN_name AS YARNQUALITY, ISNULL(COLORMASTER.COLOR_name, '') AS COLOR, ISNULL(YARNRECDDYEING_SOURCEDETAILS.YARN_WT,0) AS WT ", "", " YARNRECDDYEING_SOURCEDETAILS INNER JOIN YARNQUALITYMASTER ON YARNRECDDYEING_SOURCEDETAILS.YARN_YARNQUALITYID= YARNQUALITYMASTER.YARN_id LEFT OUTER JOIN COLORMASTER ON YARNRECDDYEING_SOURCEDETAILS.YARN_COLORID = COLORMASTER.COLOR_id ", " AND YARNRECDDYEING_SOURCEDETAILS.YARN_NO = " & TEMPYARNNO & " AND YARNRECDDYEING_SOURCEDETAILS.YARN_YEARID = " & YearId)
                If dttable.Rows.Count > 0 Then
                    For Each DTR As DataRow In dttable.Rows
                        GRIDSOURCE.Rows.Add(Val(DTR("GRIDSRNO")), DTR("YARNQUALITY"), DTR("COLOR"), Val(DTR("WT")))
                    Next
                    'ENABLE ALL COMBOBOXES
                    If GRIDSOURCE.RowCount > 1 Then
                        CHKREDYEING.CheckState = CheckState.Checked
                        TXTSOURCESRNO.Visible = True
                        CMBSOURCEYARNQUALITY.Visible = True
                        CMBSOURCECOLOR.Visible = True
                        TXTSOURCEWT.Visible = True
                        GRIDSOURCE.Enabled = True
                    End If
                End If
                getsrno(GRIDSOURCE)
        ...
        Try
            If Val(tstxtbillno.Text.Trim) > 0 Then
                ...
                If TEMPYARNNO > 0 Then
                    EDIT = True
                    YarnRecdFromDyeing_Load(sender, e)
                Else
                    CLEAR()
                    EDIT = False
                End If
    ...
    Private Sub cmddelete_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmddelete.Click
        Try
            ...
            If EDIT = True Then
                ...
                Dim Clsgrn As New ClsYarnRecdFromDyeing()
    ...
    Sub PRINTREPORT(ByVal YARNNO As Integer)
        Try
            If MsgBox("Wish to Print Yarn Recd?", MsgBoxStyle.YesNo) = vbYes Then
                ...
                OBJPUR.FRMSTRING = "YARNRECDDYEING"
                OBJPUR.WHERECLAUSE = "{YARNRECDDYEING.YARN_NO}=" & Val(YARNNO) & " and {YARNRECDDYEING.YARN_YEARID}=" & YearId
                OBJPUR.Show()
            End If
        Catch ex As Exception
            Throw ex
        ...
        Try
            ...
            If DTPO.Rows.Count > 0 Then
                ...
                For Each DTROW As DataRow In DTPO.Rows
                    ...
                    Next
                    CHKREDYEING.Checked = Convert.ToBoolean(DTROW("REDYEING"))
                    ...
                    GRIDSOURCE.Enabled = Convert.ToBoolean(DTROW("REDYEING"))
                    TXTSOURCESRNO.Visible = Convert.ToBoolean(DTROW("REDYEING"))
                    CMBSOURCEYARNQUALITY.Visible = Convert.ToBoolean(DTROW("REDYEING"))
                    CMBSOURCECOLOR.Visible = Convert.ToBoolean(DTROW("REDYEING"))
                    TXTSOURCEWT.Visible = Convert.ToBoolean(DTROW("REDYEING"))

NEXTLINE:
                Next

                If DTPO.Rows(0).Item("SOURCEYARNQUALITY") <> "" Then GRIDSOURCE.Rows.Add(0, DTPO.Rows(0).Item("SOURCEYARNQUALITY"), DTPO.Rows(0).Item("SOURCECOLOR"), 0)
    ...
    Private Sub TXTSOURCEWT_Validated(sender As Object, e As EventArgs) Handles TXTSOURCEWT.Validated
        Try
            If CMBSOURCEYARNQUALITY.Text.Trim <> "" And CMBSOURCECOLOR.Text.Trim <> "" And Val(TXTSOURCEWT.Text.Trim) > 0 Then
                FILLSOURCEGRID()
            End If
        Catch ex As Exception
            Throw ex
    ...
    Sub FILLSOURCEGRID()
        ...
        If GRIDSOURCEDOUBLECLICK = False Then
            GRIDSOURCE.Rows.Add(Val(TXTSOURCESRNO.Text.Trim), CMBSOURCEYARNQUALITY.Text.Trim, CMBSOURCECOLOR.Text.Trim, Val(TXTSOURCEWT.Text.Trim))
            getsrno(GRIDSOURCE)
        ElseIf GRIDSOURCEDOUBLECLICK = True Then
            GRIDSOURCE.Item(SSRNO.Index, TEMPSOURCEROW).Value = Val(TXTSOURCESRNO.Text.Trim)
            GRIDSOURCE.Item(SYARNQUALITY.Index, TEMPSOURCEROW).Value = CMBSOURCEYARNQUALITY.Text.Trim
            GRIDSOURCE.Item(SCOLOR.Index, TEMPSOURCEROW).Value = CMBSOURCECOLOR.Text.Trim
            GRIDSOURCE.Item(SWT.Index, TEMPSOURCEROW).Value = Format(Val(TXTSOURCEWT.Text.Trim), "0.00")
        ...
        GRIDSOURCE.FirstDisplayedScrollingRowIndex = GRIDSOURCE.RowCount - 1

        CMBSOURCEYARNQUALITY.Text = ""
        CMBSOURCECOLOR.Text = ""
        TXTSOURCEWT.Clear()
        TXTSOURCESRNO.Text = Val(GRIDSOURCE.RowCount) + 1
        CMBSOURCEYARNQUALITY.Focus()

    End Sub

    Private Sub GRIDSOURCE_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles GRIDSOURCE.CellDoubleClick
        Try
            If GRIDSOURCE.CurrentRow.Index >= 0 And GRIDSOURCE.Item(SSRNO.Index, GRIDSOURCE.CurrentRow.Index).Value <> Nothing Then
                GRIDSOURCEDOUBLECLICK = True
                TXTSOURCESRNO.Text = Val(GRIDSOURCE.Item(SSRNO.Index, GRIDSOURCE.CurrentRow.Index).Value)
                CMBSOURCEYARNQUALITY.Text = GRIDSOURCE.Item(SYARNQUALITY.Index, GRIDSOURCE.CurrentRow.Index).Value.ToString
                CMBSOURCECOLOR.Text = GRIDSOURCE.Item(SCOLOR.Index, GRIDSOURCE.CurrentRow.Index).Value.ToString
                TXTSOURCEWT.Text = Val(GRIDSOURCE.Item(SWT.Index, GRIDSOURCE.CurrentRow.Index).Value)

                TEMPSOURCEROW = GRIDSOURCE.CurrentRow.Index
                CMBSOURCEYARNQUALITY.Focus()
            End If
        Catch ex As Exception
            Throw ex
    ...
    Private Sub GRIDSOURCE_KeyDown(sender As Object, e As KeyEventArgs) Handles GRIDSOURCE.KeyDown
        Try
            If e.KeyCode = Keys.Delete And GRIDSOURCE.RowCount > 0 And CHKREDYEING.Checked = True Then
                If GRIDSOURCEDOUBLECLICK = True Then
                    MessageBox.Show("Row is in Edited Mode, You Cannot Delete This Row")
                    Exit Sub
                End If
                GRIDSOURCE.Rows.RemoveAt(GRIDSOURCE.CurrentRow.Index)
                getsrno(GRIDSOURCE)
                TOTAL()
            End If
        Catch ex As Exception
