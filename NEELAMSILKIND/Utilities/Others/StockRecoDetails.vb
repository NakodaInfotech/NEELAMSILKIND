...
Public Class StockRecoDetails
    ...
    Private Sub StockAdjustmentDetails_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        fillgrid(" AND dbo.STOCKADJUSTMENT.SA_yearid=" & YearId & " order by dbo.STOCKADJUSTMENT.SA_no ")
    End Sub
    ...
    Sub fillgrid(ByVal TEMPCONDITION)
        Try
            ...
            Dim dt As DataTable
            dt = objclsCMST.search(" STOCKADJUSTMENT.SA_no AS SRNO, STOCKADJUSTMENT.SA_date AS DATE, ISNULL(GODOWNMASTER.GODOWN_name, '') AS GODOWN,ISNULL(TRANSNAME.Acc_cmpname, '') AS TRANSNAME, ISNULL(STOCKADJUSTMENT.SA_TOTALPCS, 0) AS TOTALPCS, ISNULL(STOCKADJUSTMENT.SA_TOTALMTRS, 0) AS TOTALMTRS, ISNULL(STOCKADJUSTMENT.SA_remarks, '') AS REMARKS", "", "  STOCKADJUSTMENT LEFT OUTER JOIN GODOWNMASTER ON STOCKADJUSTMENT.SA_GODOWNID = GODOWNMASTER.GODOWN_id LEFT OUTER JOIN LEDGERS AS TRANSNAME ON STOCKADJUSTMENT.SA_TRANSID = TRANSNAME.Acc_id", Where & TEMPCONDITION)
