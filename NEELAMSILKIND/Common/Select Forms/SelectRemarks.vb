...
Public Class SelectRemarks
    ...
    Private Sub SelectQuality_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Try
            ...
            FILLGRID("")
            If FRMSTRING = "" Then FRMSTRING = "NARRATION"
    ...
    Sub FILLGRID(ByVal WHERECLAUSE As String)
        Try
            ...
            If FRMSTRING = "REASON" Then
                DT = OBJCMN.search("  ISNULL(REASON_name, '') AS REASONNAME, REASON_ID AS ID", "", "  REASONMASTER", WHERECLAUSE & "  AND REASONMASTER.REASON_YEARID = " & YearId & " order by REASONMASTER.REASON_name ")
            Else
                DT = OBJCMN.search("  ISNULL(NARRATION_name, '') AS NARRATIONNAME, NARRATION_ID AS ID", "", "  NARRATIONMASTER", WHERECLAUSE & " AND NARRATIONMASTER.NARRATION_YEARID = " & YearId & " order by NARRATIONMASTER.NARRATION_name ")
            End If
            GRIDREMARKS.DataSource = DT

            If FRMSTRING = "REASON" Then GRIDREMARKS.Columns(0).HeaderText = "Reason" Else GRIDREMARKS.Columns(0).HeaderText = "Narration"
            ...
            If TXTNAME.Text.Trim <> "" And cmbname.Text.Trim <> "" Then
                If rbstart.Checked = True Then
                    If cmbname.Text = "Name" Then
                        If FRMSTRING = "REASON" Then FILLGRID(" AND REASONMASTER.REASON_name like '%" & TXTNAME.Text.Trim & "'") Else FILLGRID(" AND NARRATIONMASTER.NARRATION_name like '%" & TXTNAME.Text.Trim & "'")
                    End If
